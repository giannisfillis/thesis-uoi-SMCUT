...
from mudpod.common import get_monte_carlo_test
...
def check_clusters_with_mudpod(data, labels, mudpod_settings, alpha= 0.01, min_size=25):
    ...
    return remapped_labels, np.array(final_centers)
...
def kruskal_plus_plus(data, options):
    ...
    mudpod_settings = options['mudpod_settings']
    ...
    for i in range(max_iter):
        ...
        print(f"Starting Mudpod Unimodality Check")
        sublabels, subcluster_centers_mudpod = check_clusters_with_mudpod(data, sublabels, mudpod_settings=mudpod_settings, alpha=alpha, min_size=options['min_size'])
        n_subclusters = subcluster_centers_mudpod.shape[0]
        options['n_subclusters'] = n_subclusters
        
        print('Refined to ' + str(n_subclusters) + ' unimodal subclusters.')
        labels, sublabels, subcluster_centers, is_active, adjacency_matrix, clusters = kruskal(data, sublabels, subcluster_centers_mudpod, options)
    ...
    def fit(self, X, y = None):
        if np.isscalar(self.options_['n_subclusters']):
            ...
            for K in self.options_['n_subclusters']:
                ...
                initial_sublabels_for_K = self.sublabels_[K]
                print(f"Starting Mudpod Unimodality Check")
                sublabels, subcluster_centers_mudpod = check_clusters_with_mudpod(data= X, labels=initial_sublabels_for_K, mudpod_settings=options['mudpod_settings'], alpha=options['alpha'], min_size=options['min_size'])
                new_k = subcluster_centers_mudpod.shape[0]
                options['n_subclusters'] = new_k
        
                self.labels_[K], self.sublabels_[K], self.subcluster_centers_[K], self.is_active_[K], self.adjacency_[K], self.clusters_[K] = kruskal(X, sublabels, subcluster_centers_mudpod, options)#self.sublabels_[K], self.subcluster_centers_[K], options)
