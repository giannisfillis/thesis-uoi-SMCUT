...
from overclustering.OverClusteringUtils import over_clustering, check_clusters_with_mudpod
...
class Smcut:
    ...
    def fit(self, X: ndarray) -> Optional[UniforceResult]:
        ...
        initial_over_clustering_result: OverClusteringResult = over_clustering(
            data=X,
        ...
        over_clustering_result = check_clusters_with_mudpod(
            data = X,
            initial_result = initial_over_clustering_result,
            mudpod_options = self._options.mudpod_options,
            min_size = self._options.spanning_tree_options.min_samples_per_sub_cluster#25
        )
        print(f"Finished Mudpod Unimodality Check")
        new_k = len(over_clustering_result.cluster_centers)
        print(f"Total unimodal subclusters after mudpod test: {new_k}")
        ...
        spanning_tree_result: MinimumSpanningTreeResult = spanning_tree(
            ...
            number_of_sub_clusters= over_clustering_result.cluster_centers.shape[0], #new_k, #self._options.number_of_clusters,
